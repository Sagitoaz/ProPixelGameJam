...
public class AngryBoss : Enemy, IDamageable
{
    ...
    [SerializeField] private GameObject _flameHitbox;
    ...
    protected void Start()
    {
        ...
        _flameHitbox.SetActive(false); // Tắt flameHitbox khi bắt đầu
    ...
    public void Damage()
    {
        // Chỉ nhận sát thương nếu không đang tấn công
        if (!_isAttack)
        {
            Health--;
            ...
            if (Health < 1)
            {
                // Dừng FlameRoutine và tắt flameHitbox
                StopCoroutine(FlameRoutine());
                _flameHitbox.SetActive(false);
                Destroy(gameObject);
            }
        }
        else
    ...
    public virtual IEnumerator FlameRoutine()
    {
        while (true)
        {
            ...
            _flameHitbox.SetActive(true);
            anim.SetTrigger("Flame"); // Kích hoạt trigger Flame trong Animator

            // Đợi cho đến khi hoạt ảnh hoàn tất (1.3 giây)
            float flameAnimationLength = 2.0f; // Độ dài dựa trên timeline
            ...
            // float remainingTime = 2.0f - flameAnimationLength;
            // if (remainingTime > 0)
            // {
